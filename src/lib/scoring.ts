...
function analyzeRhymes(bars: Bar[]): RhymeAnalysis {
  ...
  bars.forEach(bar => {
    const words = bar.text.toLowerCase().split(/\s+/).map(w => w.replace(/[^a-z]/g, ''));
...
function analyzeWordplay(bars: Bar[]): number {
  const fullText = bars.map(b => b.text).join(' ').toLowerCase();
  let score = 0;
  ...
  if (fullText.includes('but') || fullText.includes('while') || fullText.includes('whereas')) {
    score += 15;
  }
...
function analyzeRelevance(bars: Bar[], opponentName?: string): number {
  ...
  const battleWords = ['battle', 'beat', 'destroy', 'murder', 'kill', 'bury', 'defeat', 'winner', 'loser', 'weak', 'fake'];
...
function analyzeOriginality(bars: Bar[]): number {
  ...
  const cliches = ['know what i mean', 'feel me', 'you know', 'check it', 'listen up'];
  ...
  const lengthVariety = new Set(sentences.map(s => s.split(/\s+/).length)).size;
 ...
 */
export function calculateScore(bars: Bar[], opponentName?: string): AutomatedScore {
  if (bars.length !== 8) {
  ...
  const rhymeSchemeScore = Math.min(30, (rhymeAnalysis.endRhymes * 3) + (rhymeAnalysis.internalRhymes * 1.5) + (rhymeAnalysis.multiSyllabicRhymes * 2));
  const wordplayFinalScore = (wordplayScore / 100) * 25;
  const flowScore = ((flowAnalysis.syllableConsistency + flowAnalysis.rhythmScore) / 200) * 20;
  const relevanceFinalScore = (relevanceScore / 100) * 15;
  const originalityFinalScore = (originalityScore / 100) * 10;

  const total = rhymeSchemeScore + wordplayFinalScore + flowScore + relevanceFinalScore + originalityFinalScore;
